...
The module offers an interface to interact with Sumokoin daemon. For the time being, like with
wallet, the only available backend is JSON RPC.

.. code-block:: python

    In [1]: from sumokoin.daemon import Daemon

    In [2]: from sumokoin.backends.jsonrpc import JSONRPCDaemon

    In [3]: daemon = Daemon(JSONRPCDaemon(port=29734))
...
.. code-block:: python
    ...
    In [5]: from sumokoin.wallet import Wallet

    In [6]: from sumokoin.backends.jsonrpc import JSONRPCWallet

    In [7]: wallet = Wallet(JSONRPCWallet(port=29738))

    In [8]: wallet.balance()
    Out[8]: Decimal('17.642325205670')

    In [9]: txs = wallet.transfer('SutoeBob9AAGXJFM46BEwBKL1LsxgwszDLhvwTSC6nu9h6Y2VMUkdPgMX9bgdgYWNx8Zsnvond3tq3e7KGwGKbu6D7K6k4zfz8g', 10, relay=False)
...
.. code-block:: python
    ...
    In [14]: txs1 = wallet.transfer('SutoeBob9AAGXJFM46BEwBKL1LsxgwszDLhvwTSC6nu9h6Y2VMUkdPgMX9bgdgYWNx8Zsnvond3tq3e7KGwGKbu6D7K6k4zfz8g', 1, relay=False)

    In [15]: txs2 = wallet.transfer('Sutoe71zcjGgj9h22vXV4VcPRbJ9pTkSQ9n2kyJkfi2ZU4go5pkzQ453QJP3ESECxoW8cGbttCMFDTfubwygq6cWF9599Y2X8ms', 2, relay=False)
    ...
    ~/devel/sumokoin-python/sumokoin/daemon.py in send_transaction(self, tx, relay)
         10
    ...
    ~/devel/sumokoin-python/sumokoin/backends/jsonrpc.py in send_transaction(self, blob, relay)
